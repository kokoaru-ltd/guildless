...
class GlmProvider(BaseProvider):
    """Zhipu GLM provider via the OpenAI-compatible chat/completions API."""
    ...
    async def generate_json(
        ...
        *,
        ...
        try:
            ...
            raise ProviderUnavailable(
                "GLM response did not contain choices[0].message.content",
                provider=self.config.name,
                model=self.config.model,
                reason="invalid_json",
                latency_ms=latency,
        ...
        if not raw_text:
            raise ProviderUnavailable(
                "GLM response contained no content",
                provider=self.config.name,
                model=self.config.model,
                reason="invalid_json",
                latency_ms=latency,
        ...
        usage = {
            ...
            "cached_input_tokens": int(source_usage.get("prompt_tokens_details", {}).get("cached_tokens", 0) or 0),
